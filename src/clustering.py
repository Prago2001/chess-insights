...
# UMAP is optional - fallback to t-SNE if not available
try:
    import umap
    UMAP_AVAILABLE = True
except ImportError:
...
def prepare_clustering_data(player_features: pd.DataFrame) -> Tuple[pd.DataFrame, List[str]]:
    ...
    exclude_cols = ['player', 'game_count', 'skill_tier', 'elo']
...
def perform_clustering(X: pd.DataFrame,
    ...
    # Using 2 components based on K-optimization analysis (silhouette 0.36 vs 0.15 without PCA)
    ...
    if compute_embedding:
        if UMAP_AVAILABLE:
            ...
            print("UMAP not available; using t-SNE for embedding")
            tsne = TSNE(
                n_components=2,
                random_state=RANDOM_STATE,
...
def analyze_clusters(player_features: pd.DataFrame,
    ...
    for cluster_id in sorted(df['cluster'].unique()):
        ...
        stats = {
            ...
            'elo': cluster_data['elo'].mean() if 'elo' in cluster_data.columns else np.nan,
            'avg_games': cluster_data['game_count'].mean() if 'game_count' in cluster_data.columns else np.nan,
        }

        # Skill tier distribution (3 tiers)
...
def name_clusters(cluster_stats: pd.DataFrame,
    ...
    """
    Name clusters based on behavioral characteristics.

    Archetype names (from proposal):
    - Time Scramblers: Fast play, comfortable in time pressure (highest Elo)
    - Tactical Battlers: Complex positions, material imbalances (lowest Elo)
    - Positional Grinders: Active pieces, controlled positions (medium Elo)
    """
    df = player_features.copy()
    df['cluster'] = labels
    ...
    # Collect cluster info for naming
    cluster_info = []
    for _, row in cluster_stats.iterrows():
        cid = int(row['cluster'])
        cluster_data = df[df['cluster'] == cid]
        size = int(row['size'])
        elo = float(row['elo']) if pd.notna(row['elo']) else 0
        cluster_info.append({'cid': cid, 'size': size, 'elo': elo, 'data': cluster_data})

    # Sort by Elo to assign names (highest Elo = Time Scramblers, lowest = Tactical Battlers)
    cluster_info.sort(key=lambda x: x['elo'], reverse=True)

    # Archetype definitions aligned with proposal
    archetype_defs = [
        {
            'name': 'Time Scramblers',
            'description': 'Fast players who thrive under time pressure',
            'characteristics': ['Fast moves', 'High time pressure comfort', 'Highest skill level']
        },
        {
            'name': 'Positional Grinders',
            'description': 'Players who maintain active pieces in controlled positions',
            'characteristics': ['Active pieces', 'Simpler positions', 'Medium skill level']
        },
        {
            'name': 'Tactical Battlers',
            'description': 'Players who seek complex tactical positions',
            'characteristics': ['Complex positions', 'Material imbalances', 'Developing skills']
        }
    ]

    # Assign names based on Elo ranking
    for i, info in enumerate(cluster_info):
        cid = info['cid']
        archetype = archetype_defs[min(i, len(archetype_defs) - 1)]
...
if __name__ == "__main__":
    ...
    if player_path.exists():
        ...
        # Evaluate k in [2, 5] - K=3 with PCA=2 gives best silhouette (0.36)
        k_results = find_optimal_k(X.values, k_range=(2, 5))
        optimal_k = 3
        print(
            f"Using n_clusters={optimal_k} for behavioral clustering "
            "based on K-optimization with PCA (silhouette=0.36)."
        )
